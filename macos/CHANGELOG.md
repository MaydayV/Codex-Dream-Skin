...
- 浅色自定义主题的页面、侧栏、面板、文字、卡片与输入框现在完整读取 `theme.json` 配色，不再只替换强调色
- 浅色横幅改用主题面板色生成可读遮罩，文字颜色跟随主题，宽幅浅色背景不再被固定白字限制
- 悬停、选中、卡片图标和输入框装饰统一跟随主题色，移除固定粉红 / 红色残留

### 修复

- 默认主题重新引用仓库内置的 `portal-hero.png`，修复全新克隆后 payload 校验因缺图失败
